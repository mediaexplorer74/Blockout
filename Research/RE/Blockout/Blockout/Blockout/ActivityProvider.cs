// Decompiled with JetBrains decompiler
// Type: Blockout.ActivityProvider
// Assembly: Blockout, Version=1.2.0.0, Culture=neutral, PublicKeyToken=null
// MVID: A5FFAAE6-A6F2-4299-B64C-2405C1773121
// Assembly location: C:\Users\Admin\Desktop\RE\BlockOut_V1\Blockout.exe

using Microsoft.Xna.Framework;

#nullable disable
namespace Blockout
{
  internal sealed class ActivityProvider
  {
    internal static bool DisconnectProject(Vector2 _param0, Vector2 _param1) => _param0 != _param1;
  }
}
